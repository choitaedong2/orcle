/*
    * ALTER : 객체를 변경하는 구문
    ...
    [표현법]
    ALTER 객체(TABLE, VIEW) 객체명(TABLE_NAME, VIEWER) 변경할 내용;
...
/*
    1.2 컬럼 수정 (MODIFY)
        ...
        - DEFAULT 값 수정
         MODIFY 컬럼명 DEFAULT 바꾸ㅈ고자하는 기본값
*/
-- 오류 : 컬럼값에 문자가 포함되어 있음. 컬럼의 타입을 변경하려면 값들을 모두 지워야 변경가능
ALTER TABLE DEPT_COPY MODIFY DEPT_ID CHAR(3);

-- 오류: 컬럼의 타입을 변경하려면 값들을 모두 지워야 변경가능
ALTER TABLE EMP_COPY MODIFY EMP_ID NUMBER;

-- 오류 : 컬럼값이 10 BYTE가 넘는 자료가 들어가 있음
...
/*
    - 외래키의 부모테이블은 삭제 안됨
      그래도 삭제하고 싶다면
      * 방법1 : 자식 테이블을 먼저 삭제한 후 부모 테이블 삭제
