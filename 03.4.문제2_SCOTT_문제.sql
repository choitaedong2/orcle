...
SELECT EMPNO, HIREDATE, ENAME, SAL FROM EMP ORDER BY DEPTNO, HIREDATE;
9. 오늘 날짜에 대한 정보 조회
...
SELECT ENAME, TO_CHAR(HIREDATE, 'RRRR"년 "MM"월"') "입사일" FROM EMP;
13. EMP테이블에서 9월에 입사한 직원의 정보 조회
SELECT * FROM EMP WHERE TO_CHAR(HIREDATE, 'MM') = '09';
14. EMP테이블에서 81년도에 입사한 직원 조회
SELECT * FROM EMP WHERE TO_CHAR(HIREDATE, 'RRRR') = '1981';
...
SELECT * FROM EMP WHERE CEIL(SYSDATE-HIREDATE)/365 >= 38;
19. 오늘 날짜에서 년도만 추출
SELECT TO_CHAR(SYSDATE, 'YY"년도"') FROM DUAL;
