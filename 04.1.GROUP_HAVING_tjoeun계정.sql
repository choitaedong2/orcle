/*
    <GROUP BY절>
    여러개의 값드을 하나의 그룹으로 묶어서 처리할 목적으로 사용
*/
...
SELECT COUNT(*), COUNT(BONUS), SUM(SALARY), ROUND(AVG(SALARY), -4), MIN(SALARY), MAX(SALARY)
...
-----------------------------------1. UNION-------------------------------------
-- 부서코드가 DS인 사원 또는 급여 300만원 초과인 사원들 조회
-- UNION
...
-- OR
 ...
 WHERE SALARY > 3000000;
 -- 집합연산자 사용시 주의사항
 -- 각 쿼리문의 SELECT절에 작성되는 동일한 컬럼이어야 한다
