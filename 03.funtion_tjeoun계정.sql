...
FROM DUAL; -- 오라클에서 제공하는 가장 테이블 'DUAL' 사용 가능
...
SELECT INSTR('JAVASCRIPTJAVAORACLE', 'A', 1, 3) FROM DUAL;-- 1번부터 찾는데 순번이 3번째인 값의 위치 찾기
SELECT INSTR('JAVASCRIPTJAVAORACLE', 'A', -1, 2) FROM DUAL;-- 뒤에서부터 찾는데 순번이 2번째인 값의 위치 찾기
...
-- EMPLOYEE테이블에서 EMAIL에서 아이디만 추출하여 차원명, 이메일, 아이디를 조회
...
/*
    * LPAD / RPAD : 문자열을 조회할 때 통일감있게 자리수에 맞춰서 조회하고자 할 때(반환형 : CHARACTER)
      ...
      LPAD/RPAD('문자열', 최종적으로 반환할 문자의 길이, [덧불이고자하는 문자])
...
-- EMPLOYEE테이블에서 주민번호를 1234567-1****** 형식 사번, 사원명, 주민번호(형식에 맞춰) 조회
...
/*
    ...
    * TRIM : 문자열의 앞/뒤 양쪽에 있는 특정 문자를 제거한 나머지 반환
    ...
    TRIM([LEADING/TRAILING/BOTH] 제거하고자하는 문자를 FROM '문자열')
*/

SELECT LTRIM('    TJOEUN     ')||'더조은' FROM DUAL; -- 제거할 문자를 안넣으면 공백제거
SELECT RTRIM('    TJOEUN     ')||'더조은' FROM DUAL; -- 제거할 문자를 안넣으면 공백제거

SELECT LTRIM('JAVAJAVASCRIPTJSP', 'JAVA') FROM DUAL;
SELECT LTRIM('BABACBBABADFSIA', 'ABC') FROM DUAL;
SELECT LTRIM('12324352341424DKI2637', '0123456789') FROM DUAL;
SELECT RTRIM('12324352341424DKI2637', '0123456789') FROM DUAL;

SELECT TRIM('    TJOEUN     ')||'더조은' FROM DUAL; -- 제거할 문자를 안넣으면 공백제거

SELECT TRIM(LEADING 'A' FROM 'AAAVEFGEFAAA') FROM DUAL;
SELECT TRIM(TRAILING 'A' FROM 'AAAVEFGEFAAA') FROM DUAL;
SELECT TRIM(BOTH 'A' FROM 'AAAVEFGEFAAA') FROM DUAL;
SELECT TRIM('A' FROM 'AAAVEFGEFAAA') FROM DUAL; -- BOTH 기본값 생략가능
-------------------------------------------------------------------------------
/*
    * LOWER / UPPER / INITCAP : 문자열을 모두 대문자로 혹은 소문자로, 첫 글자만 대문자로 변환(반환형 : CHARACTER)
...
SELECT EMP_NAME, EMAIL, REPLACE(EMAIL, 'tjoeun.or.kr', 'naver.com') FROM EMPLOYEE;
--=============================================================================
--                            <숫자 처리 함수>
--=============================================================================
/*
...
SELECT ABS(-34.324234) FROM DUAL;
-------------------------------------------------------------------------------
/*
    * MOD : 두 수를 나눈 나머지
...
/*
    * ROUND : 반올림한 결과를 반환해주는 함수
    ...
    ROUND(NUMBER, [위치])
*/
SELECT ROUND(1234.5678) FROM DUAL; -- 위치 생략시 0
...
-------------------------------------------------------------------------------
/*
    * EXTRACT : 특정 날짜로 부터 년/월/일 값을 추출하여 반환해주는 함수(반환형 : NUMBER)
...
/*
    ...
    FM : 해당 자리에 값이 없을 경우 자리를 차리하지 않음.
*/
...
SELECT TO_CHAR(12341234, 'L999,999,999') FROM DUAL; -- 3자리마다 ,(콤마)

-- EMPLOYEE테이블에서 사원명, 급여(L11,111,111), 연봉(L111,111,111)
...
-- EMPLOYEE테이블에서 입사일(25-03-20)으로 조회
SELECT EMP_NAME, TO_CHAR(HIRE_DATE, 'yy"-"MM"-"DD')입사일 FROM EMPLOYEE;

-- EMPLOYEE테이블에서 입사일(2024년 3월 20일 화요일)으로 조회
SELECT EMP_NAME, TO_CHAR(HIRE_DATE, 'DL')입사일 FROM EMPLOYEE;

-- EMPLOYEE테이블에서 입사일(2024년 03월 20일 화요일)으로 조회
SELECT EMP_NAME, TO_CHAR(HIRE_DATE, 'yy"년 "MM"월 "DD"일"')입사일 FROM EMPLOYEE;
...
-- 일
SELECT TO_CHAR(SYSDATE,'DDD') -- 연을 기준으로 몇일째
    , TO_CHAR(SYSDATE, 'DD') -- 월을 기준으로 몇일째
    , TO_CHAR(SYSDATE, 'D') -- 일을 기준으로 몇일째
FROM DUAL;
...
SELECT TO_DATE('050101') FROM DUAL; -- 앞이 9일때는 문자형태로 넣어줘야 한다.

--SELECT TO_DATE('070204 142530', 'YYMMDD HHMISS') FROM DUAL; -- 오류 12시간제로 출력 14시는 없음
SELECT TO_DATE('070204 142530', 'YYMMDD HH24MISS') FROM DUAL; -- 24시간제로 변경해도 똑같음. 시간 표기는 변환후 사용 필요
...
/*
    * TO_NUMBER: 문자 타입을 숫자타입으로 변환
    ...
    TO_NUMBER(문자, [포맷])
*/
SELECT TO_NUMBER('012341234') FROM DUAL; -- 숫자로 변경했기 때문에 '0'으로 시작시 '0'은 공백
SELECT '1000000' + '5000000' FROM DUAL; -- 오라클은 자동 형변환 됨
SELECT '1,000,000' + '5,000,000' FROM DUAL; -- 오류 : 특수문자가 포함되어 문자임. 자동 형변환 안됨
...
SELECT EMP_NAME, SALARY*NVL(1+BONUS, 1)*12 FROM EMPLOYEE;

-- 사원명, 부서코드(부서가 없으면 '부서 없음'으로)
SELECT EMP_NAME, NVL(DEPT_CODE, '부서 없음') FROM EMPLOYEE;
-------------------------------------------------------------------------------
/*
    * NVL2(컬럼, 반환값1, 반환값2)
     - 컬럼에 값이 존재하면 반환값1
     - 컬럼에 값이 존재하지 않으면(NULL) 반환값2
*/
-- 사원명, 부서(부서에 속해 있으면 '부서 있음', 부서가 없으면 '부서 없음')
...
*/
SELECT EMP_ID, EMP_NAME, EMP_NO, DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여', '3', '남', '4', '영') 성별
FROM EMPLOYEE;
...
/*
    * CASE WHEN THEN
      ...
      ELSE (실행문)
*/
-- 사원명, 급여, 등급 (급여에 따른 등급을 매긴다(5백만원 이상이면 '고급' 5백~3백 '중급' 나머지 '초급'))
...
-------------------------------------------------------------------------------
/*
    * MIN(모든타입의 컬럼) : 해당 컬럼값들 중 가장 작은 값을 반환해 주는 함수
...
/*
    * COUNT(*/컬럼/DISTINCT 컬럼) : 행 갯수 반환
    ...
    COUNT(DISTINCT 컬럼) : 컬럼값에서 중복을 제외한 행의 갯수 반환
*/
-- 전체 사원의 수
...
-- 보너스를 받는 사원의 수
SELECT COUNT(BONUS) "보너스 받는 사람의 수" FROM EMPLOYEE;

-- 현재 사원들이 총 몇개의 부서에 분포되어 있는지 갯수 조회
