...
/*
    * DML 명령어로 조작이 불가능한 경우
     ...
     2) 뷰에 정의되어 있는 컬럼중에 테이블에 NOT NULL제약 조건이 지정되어 있는 경우
...
-- INSERT
INSERT INTO VW_SAL VALUES(400, '아무개','901234-1234567', 'J1',3000000, 300000);
-- 오류 : 산술연산식이 포함되어 있음

-- UPDATE, DELETE 가능
-- 사번 302인 사원의 연봉을 3000000
UPDATE VW_SAL
SET 연봉 = 30000000
WHERE EMP_ID = 302;
-- 오류
...
-- INSERT (오류)
INSERT INTO VW_GROUP VALUES('D3', 80000000, 4000000);
...
-- 5) DISTINCT 구문이 포함된 경우 (GROUP BY와 동일 : 한 그룹에 여러명이 포함)
-- 6) JOIN을 이용하여 여러 테이블을 연결시켜놓은 경우
...
/*
    * VIEW 옵션
    ...
    - FORCE | NOFORCE
     ...
     > NOFORCE(기본형) : 서브쿼리에 기술된 테이블이 반드시 존재 해야만 뷰를 생성할 수 있다.
...
-- 301번을 2000000만원으로 변경(CHECK OPTION으로 3,000,000만원 이하는 변경 불가)
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID = '301';
