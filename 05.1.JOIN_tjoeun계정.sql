/*
    ...
    - JOIN은 크게 "오라클전용 구문"과 "ANSI 구문" (ANSI == 미국국립표준협회)
    ...
    -------------------------------------------------------------------------
        카테시안곱(CARTESIAN RPODUCT) |          교차 조인(CROSS JOIN)
*/
...
-- 각 부서명 별 전체 EMPLOYEE 적용(인사관리부 23번, 회계관리부 23번... )
...
-- 2) 연결할 두 컬럼이 서로 같은 경우(EMPLOYEE: JOB_CODE, JOB: JOB_CODE)
-- 전체 사원의 사번, 사원명, 직급코드, 직급명을 조회
-- SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
-- FROM EMPLOYEE, JOB WHERE JOB_CODE;

-- 해결방법 2-1) 테이블명을 이용하는 방법
SELECT EMP_ID, EMP_NAME, EMPLOYEE.JOB_CODE, JOB_NAME
FROM EMPLOYEE, JOB WHERE EMPLOYEE.JOB_CODE = JOB.JOB_CODE;

-- 해결방법 2-2) 테이블에 별칭을 부여하여 이용하는 방법
...
--      JOIN절에 같이 조회하고자 하는 테이블 기술 + 매칭시킬 컬럼에 대한 조건 제시
...
-- 해결방법 2-1) 테이블에 테이블명 또는 별칭을 이용하는 방법
SELECT EMP_ID, EMP_NAME, E.JOB_CODE, JOB_NAME
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE);

-- 해결방법 2-2) JOIN USING 구문 사용하는 방법(두 컬럼명이 일치할 때만 사용가능)
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
...
*/
-- 사원명, 부서명, 급여, 연봉(부서를 배치받지 못한 사원도 조회)
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12 연봉
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- 부서배치가 안된 사원 2명은 제외
...
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12 연봉
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+); -- 기준이 되는 테이블 반대편 테이블 컬럼의 뒤에(+)를 붙여줌
...
-- 3) Full [OUTER] JOIN : 두 테이블이 가진 모든 행 조회(오라클 전용구문 사용 불가)
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12 연봉
FROM EMPLOYEE
...
/*
    * 비등가 조인
      ...
      ANSI구문으로는 JOIN ON으로만 가능(=USING 사용 불가)
*/
...
*/
-- 전체 사원의 사번, 사원명, 부서코드, 사수사번, 사수명, 사수부서코드
-- >> 오라클 전용 구문
-- 사수가 있는 사원만 조회
SELECT E.EMP_ID, E.EMP_NAME, E.DEPT_CODE, M.EMP_ID 사수사번, M.EMP_NAME 사수명, M.DEPT_CODE 사수부서코드
...
--(EMPLOYEE, DEPARTMENT, LOCATION, NATIONAL 조인)
-- >> 오라클 구문
...
FROM EMPLOYEE E, JOB J, DEPARTMENT, LOCATION L, NATIONAL N, SAL_GRADE
WHERE SALARY >= MIN_SAL AND SALARY <= MAX_SAL
    AND E.JOB_CODE = J.JOB_CODE
